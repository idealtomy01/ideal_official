...
## 📋 基本情報
- **分析対象**: 行194-424
- **データタイプ**: AccordionItem[]
- **使用場所**: Accordionコンポーネント
...
#### 工程2: ブロックチェーンによる『記録に基づく計算』と記録の保全
```tsx
{
  id: 'step-2',
...
#### 工程3: スマートコントラクトによるインセンティブの『自動執行』
```tsx
{
  id: 'step-3',
...
#### 工程4: 時間と範囲を超えた『影響の追跡』と『永続的フィードバック』
```tsx
{
  id: 'step-4',
...
```tsx
<ul className="list-disc list-inside ml-4 space-y-2">
  <li><strong>項目名:</strong> 説明テキスト</li>
</ul>
```
...
### Accordion
- **processSteps**: セクション3で使用
- **philosophyItems**: セクション2で使用
- **roadmapItems**: セクション5で使用（青枠付き）

## 🎯 重要なポイント

1. **複雑なコンテンツ**: 各アコーディオン項目に詳細なJSXコンテンツ
2. **特別なスタイリング**: 背景色、境界線、下線などの装飾
3. **リスト構造**: 番号付きリストと箇条書きリスト
4. **青いテキスト**: ロードマップの「フェーズX」テキスト
5. **データの分離**: アコーディオンデータとコンテンツの分離
